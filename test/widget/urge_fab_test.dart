...
void main() {
  testWidgets('urge FAB is present and opens the toolkit flow', (tester) async {
    ...
    // The signal-coloured urge button is on the dashboard.
    expect(find.text('Verlangen'), findsOneWidget);

    // Tapping it opens the toolkit at the initial-intensity step. (We stop
    // before "Los", which would write to the DB, to keep the test hermetic.)
    await tester.tap(find.text('Verlangen'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 400));
    expect(find.text('Wie stark ist das Verlangen gerade?'), findsOneWidget);
